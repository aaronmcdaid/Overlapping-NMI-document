...
(the cluster in $Y$ can be encoded with zero bits because it has a perfect match among the clusters of $X$)
...
in these cases would be for a similarity score close to $0$. If we remove the
normalization from LFKNMI, and instead use a more conventional normalization strategy
\cref{eqnNorms}, this more intuitive behaviour will be found.
